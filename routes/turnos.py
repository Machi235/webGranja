...
def horarios():
    ...
    cur.execute("SELECT * FROM usuarios AS u LEFT JOIN usuarioturno AS t ON u.idUsuario = t.idUsuario WHERE t.idUsuario IS NULL AND rol != 'Admin';")      
...
def asignar_turno():
    ...
    cur = conn.cursor()
    cur.execute("""
        INSERT INTO usuarioturno (idHorario, idUsuario, activo)
        VALUES (%s, %s, 1)
